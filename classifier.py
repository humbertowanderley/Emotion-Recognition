...
def classifyEmotion(feat):
    ...
    X = np.asarray(feat)
    return [int(clf_neutral.predict(X.reshape(1,-1))[0]),int(clf_happy.predict(X.reshape(1,-1))[0]),int(clf_sad.predict(X.reshape(1,-1))[0]),int(clf_surprise.predict(X.reshape(1,-1))[0]),int(clf_angry.predict(X.reshape(1,-1))[0]),int(clf.predict(X.reshape(1,-1))[0])]
